...
def network_model():
    ...
    x = p
    x = Dense(width, activation='relu', use_bias=True)(x)
...
if not run_train or load_existing:
    model.load_weights(f'models/cnn') 
...
if run_train:
    ...
    pickle_save(history.history, f'models/cnn_history.pkl')
...
np.save(f'data/cnn_pred.npy', snapshot)
...
np.save(f'data/cnn_c.npy', c_pred)
...
ax.legend()
plt.savefig(f'images/cnn_pred.png', dpi=300)
...
snapshot = np.expand_dims(snapshot, -1)
np.save(f'data/cnn_pred_newic.npy', snapshot)
...
c_pred = np.expand_dims(c_pred, -1)
np.save(f'data/cnn_c_newic.npy', c_pred)
...
ax.legend()
plt.savefig(f'images/cnn_pred_newic.png', dpi=300)
