...
x_train, x_val, p_train, p_val, y_train, y_val = train_test_split(dtrain[:, :36], dtrain[:, [36]], dtrain[:,37:], train_size=0.9, random_state=seed)
...
if run_train:
    ...
    model.save(f'models/mlp') 
    pickle_save(history.history, f'models/mlp_history.pkl')
...
for z in z_test:
    for nx in nx_list:
        ...
        np.save(f'data/mlp_pred{z}_{nx}.npy', pred_test)
        scipy.io.savemat(f'data/mlp_pred{z}_{nx}.mat', dict(dg_data=pred_test))