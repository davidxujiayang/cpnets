...
z_plus_list = 650:50:850;

for i = 1:length(z_plus_list)
    z_plus = z_plus_list(i);
    for Nxdg = [8 16 24 32 48 64]
        gen_data(z_plus, Nxdg)
    end
end

function gen_data(z_plus, Nxdg)
    disp(z_plus)
    disp(Nxdg)
    snap_fn = "data/snap" + z_plus + ".mat";
    load(snap_fn);

    energy_modes = zeros(Nyplus, 16);
    energy_modes2 = zeros(Nyplus, 16);
    Re_rms = zeros(Nyplus, 1);

    counter = 1;

    Nzdg = Nxdg / 2; %Number of Elements in y
    p_DG = 1; %Number of Elements in z
    ...
    for ypi = 1:Nyplus
        ...
        for i = 1:Nxdg
            ...
            for j = 1:Nzdg
                ...
                coeff4 = squeeze(L2_solU2(ypi, i, j, :));

                coeff4m_nrm = n2m_matrix2 * (coeff4 - mean2) / rms2;

                for k = 1:16
                    energy_modes2(ypi, k) = energy_modes2(ypi, k) + (coeff4m_nrm(k).^2) / (Nxdg * Nzdg);
                end

                Re_rms(ypi) = Re_rms(ypi) + dx_dg * rms2 / (nu * Nxdg * Nzdg);
    ...
    for ypi = 1:Nyplus
        ...
        for i = 1:Nxdg
            ...
            for j = 1:Nzdg
                ...
                coeff2r = (coeff2 - mean2) / rms2;
                coeff4r = (coeff4 - coeff2i) / rms2;

                dg_data(counter, :) = [coeff2r log(dx_dg * rms2 / nu) coeff4r];
    ...
    dg_fn = "data/dg" + z_plus + "_" + Nxdg + ".mat";
    save(dg_fn, 'dg_data');
end
