...
filename = f'cpgnet-w{width}-d{depth}-n{nstd}-nt{ntrain}'
print(filename)
...
def get_model(width, depth):
    ...
    e = E_in
    e = ConditionedDense(10, use_bias=True, name=f'input_e_conddense_0')([e, e])
    e = ConditionedDense(4, use_bias=True, name=f'input_e_conddense_1')([e, e])
    
    e_inner = tf.gather(e, edge_inner, axis=1)
    e_noslip = tf.gather(e, edge_noslip, axis=1)
    e_sym = tf.gather(e, edge_sym, axis=1)
    
    x = ConditionedDense(width, use_bias=True, name=f'input_x_conddense_0')([x, x])
    x = LayerNormalization()(x)
    x = ConditionedDense(width, use_bias=True, name=f'input_x_conddense_1')([x, x])
    x = LayerNormalization()(x)

    for i in range(depth):
        x0 = x

        x = Dense(2*width, activation='relu', name=f'x_dense_{i}')(x)
        x = LayerNormalization()(x)

        xi = tf.gather(x, index_i_inner, axis=1) 
        xj = tf.gather(x, index_j_inner, axis=1)

        concat = Concatenate()([xi, xj])
        flux_inner = ConditionedDense(width, use_bias=True, name=f'flux_conddense_{i}')([concat, e_inner])
        x_noslip = tf.gather(x, index_i_noslip, axis=1)
        flux_noslip = ConditionedDense(width, use_bias=True, name=f'flux_noslip_conddense_{i}')([x_noslip, e_noslip])
        x_sym = tf.gather(x, index_i_sym, axis=1)
        flux_sym = ConditionedDense(width, use_bias=True, name=f'flux_sym_conddense_{i}')([x_sym, e_sym])
        ...
        s = ConditionedDense(width, use_bias=True, name=f'source_conddense_{i}')([x, x])
    ...
    x_dec = X_in
    x_dec = ConditionedDense(width, use_bias=True, name=f'output_x_conddense_0')([x_dec, x_dec])
    x_dec = LayerNormalization()(x_dec)
    x_dec = ConditionedDense(width//2, use_bias=True, name=f'output_x_conddense_1')([x_dec, x_dec])
    r = ConditionedDense(nvar, use_bias=True, name=f'output_r_conddense')([r, x_dec])
...
np.save(f'data/cpgnet_pred.npy', fs.denormalize(np.concatenate((nf[[ntrain],], nf_pred), axis=0)[::10,])) # includes the IC
